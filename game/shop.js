...
(function() {
  ...
  function createShopPanel() {
    ...
    shopContainer.innerHTML = `
      ...
      <div class="shop-window">
        ...
        <h2 class="shop-title">🛒 Block Blast Shop</h2>
  ...
  function renderCoinShop(container) {
    ...
    html += '</div></div><div class="shop-section"><h3>🎲 Block Styles</h3><div class="shop-grid">';
    for (const s of catalog.pieceStyles) {
      ...
      html += `<div class="shop-item ${owned ? 'owned' : ''} ${active ? 'active' : ''}" data-type="pieceStyle" data-id="${s.id}" data-price="${s.price}">
    ...
    for (const p of catalog.powerupPacks) {
      html += `<div class="shop-item" data-type="powerup" data-id="${p.id}" data-price="${p.price}">
    ...
    for (const b of catalog.boosters) {
      html += `<div class="shop-item" data-type="booster" data-id="${b.id}" data-price="${b.price}">
    ...
    container.querySelectorAll('.btn-buy').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        handleCoinPurchase(item.dataset.type, item.dataset.id, parseInt(item.dataset.price), item);
      });
    });
    container.querySelectorAll('.btn-equip').forEach(btn => {
      btn.addEventListener('click', (e) => {
        const item = e.target.closest('.shop-item');
        handleEquip(item.dataset.type, item.dataset.id);
      });
    });
  }
  ...
  function handleCoinPurchase(type, id, price, itemEl) {
    ...
    if (type === 'theme') { state.ownedThemes.push(id); state.activeTheme = id; ProgressionSystem.save(); }
      ...
      if (pack) { for (const [k, v] of Object.entries(pack.items)) { state.powerups[k] = (state.powerups[k] || 0) + v; } ProgressionSystem.save(); }
  ...
  function renderGemShop(container) {
    ...
    for (const p of packs) {
      ...
      html += `<div class="shop-item gem-pack ${p.popular ? 'popular' : ''}" data-id="${p.id}">
    ...
    container.querySelectorAll('.gem-pack .btn-buy').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        const data = ProgressionSystem.getGemPacks().find(p => p.id === pack.dataset.id);
        if (!data) return;
        ProgressionSystem.addGems(data.gems + data.bonus);
        showNotification(`💎 Purchased ${data.gems + data.bonus} gems!`);
        updateBalances();
      });
    });
  }
  ...
  function renderUpgradeStation(container) {
    ...
    html += `<div class="bonus-summary">
      ...
      <span>💰 Row Bonus: <strong>+${bonuses.rowBonus}</strong></span>
      <span>📦 Grid: <strong>+${bonuses.gridBonus}</strong></span>
      <span>🔥 Combo: <strong>+${bonuses.comboBonus}</strong></span>
      <span>📋 Slots: <strong>+${bonuses.extraSlot}</strong></span>
    </div>`;
    ...
    for (const [cat, tier] of Object.entries(tiers)) {
      ...
      html += `</div></div>`;
      if (currentData) {
        html += `<div class="upgrade-current">Current: <strong>${currentData.name}</strong></div><div class="upgrade-bonuses">`;
        for (const [k, v] of Object.entries(currentData.bonus)) { if (v > 0) html += `<span class="bonus-chip">${v > 1 ? (k === 'scoreMult' ? `${v}x` : v) : ''} ${formatBonusName(k)}</span>`; }
        html += `</div>`;
      }
      if (nextData) {
      ...
      if (maxed) { html += `<div class="upgrade-maxed">⭐ MAX LEVEL ⭐</div>`; }
        ...
        html += `<div class="upgrade-actions">
          <button class="btn-upgrade coin-upgrade ${state.coins >= costs.coins ? '' : 'disabled'}" data-cat="${cat}" data-currency="coins">🪙 ${costs.coins.toLocaleString()}</button>
          <button class="btn-upgrade gem-upgrade ${state.gems >= costs.gems ? '' : 'disabled'}" data-cat="${cat}" data-currency="gems">💎 ${costs.gems}</button>
        </div>`;
      }
      html += `</div>`;
    }
    ...
    container.querySelectorAll('.btn-upgrade:not(.disabled)').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        const result = ProgressionSystem.upgradeItem(card.dataset.cat, btn.dataset.currency === 'gems');
        if (result.success) { showNotification(`⬆️ ${card.dataset.cat} upgraded to Lv.${result.newLevel}!`); renderUpgradeStation(container); updateBalances(); }
  ...
  function renderPremiumShop(container) {
    ...
    const state = ProgressionSystem.getState();
    let html = '<div class="shop-section"><h3>👑 Premium Shop</h3><p class="shop-subtitle">Exclusive items — real money only</p>';
    html += '<h4>⚔️ Legendary Block Skins</h4><div class="shop-grid">';
    for (const s of premium.legendarySkins) {
      const owned = state.inventory[s.id];
      html += `<div class="shop-item premium-item ${owned ? 'owned' : ''}">
    ...
    for (const b of premium.bundles) {
      ...
      html += `<div class="shop-item premium-item bundle-item ${owned ? 'owned' : ''}">
        <div class="item-name">${b.name}</div>
        <div class="item-desc">${b.desc}</div>
    ...
    for (const p of premium.premiumCases) {
      ...
      html += `<div class="shop-item premium-item subscription-item ${owned ? 'owned' : ''}">
        <div class="item-name">${p.name}</div>
        <div class="item-desc">${p.desc}</div>
    ...
    html += '</div><h4>🚫 Ads</h4><div class="shop-grid">';
    const adFree = state.adFree;
    html += `<div class="shop-item premium-item ${adFree ? 'owned' : ''}">
    ...
    container.querySelectorAll('.iap-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        if (confirm(`Purchase this item for ${fmtPrice(price)}? (Real money — simulated)`)) {
  ...
  function formatBonusName(key) {
    const names = { scoreMult: 'Score', clearBonus: 'Clear Bonus', gridBonus: 'Grid Bonus', rowBonus: 'Row Bonus', comboBonus: 'Combo', extraSlot: 'Extra Slot' };
